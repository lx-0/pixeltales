...
export function SceneProposalList({
  ...
  const handleVote = async (proposalId: number, vote: number) => {
    try {
      await voteMutation.mutateAsync({ sceneId: proposalId, vote });
      addVote(proposalId);
      toast({
              ...
              {proposals.map((proposal) => {
                const id =
                  typeof proposal.id === 'string'
                    ...
                    : proposal.id;
                const hasVoted = id ? votedProposals.has(id) : false;
