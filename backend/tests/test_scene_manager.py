...
@pytest.fixture
def sm(scene: Scene) -> SceneManager:
    """SceneManager with its active scene pre-populated (skips DB load)."""
    m = SceneManager()
    m.scene = scene
...
class TestVisitorTracking:
    async def test_add_visitor_increments_count(self, sm: SceneManager):
    ...
    async def test_remove_visitor_decrements_count(self, sm: SceneManager):
        await sm.add_visitor("sid-1")
    ...
    async def test_last_visitor_leaves_pauses_conversation(self, sm: SceneManager):
        await sm.add_visitor("sid-1")
        await sm.remove_visitor("sid-1")
        assert sm.scene is not None
...
class TestNextSpeaker:
    def test_empty_conversation_starts_with_configured_character(self, sm: SceneManager):
        assert sm.scene is not None
        sm.scene.state.messages = []
        assert sm._get_next_speaker() == sm.scene.config.start_character_id == "alice"

    def test_alternates_from_last_speaker(self, sm: SceneManager):
        assert sm.scene is not None
...
class TestGetOtherCharacter:
    def test_returns_the_other_character_when_only_two(self, sm: SceneManager):
...
class TestSetVisitors:
    def test_active_when_visitors_present(self, sm: SceneManager):
        assert sm.scene is not None
        state = sm._set_visitors(3, sm.scene.state)
        assert state.visitor_count == 3
        assert state.conversation_active is True

    def test_paused_without_visitors(self, sm: SceneManager):
        assert sm.scene is not None
...
class TestStart:
    async def test_start_is_idempotent(self, sm: SceneManager):
