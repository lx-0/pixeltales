 ...
 */
@Injectable()
export class OpenAIImageLlmService extends BaseImageLlmService implements OnModuleInit {
  ...
  constructor(
    ...
    super(OpenAIImageLlmService.name, _logger);
  }

  onModuleInit() {
    // Initialization logic moved here
    // Base class likely handles context setting
    this.logger.setContext(OpenAIImageLlmService.name);
    this.logger.info('Initializing OpenAIImageLlmService via onModuleInit...');
    this.apiKey = this.getApiKey('openai'); // Get API key using configService
    this.client = new OpenAI({ apiKey: this.apiKey }); // Initialize client
    this.logger.info(`Initialized OpenAI client with model ${this.model} for image generation`);
  }
  ...
  async analyzeImage(options: ImageAnalysisOptions): Promise<ImageAnalysisResult> {
    this.logger.info(`🔍 Analyzing image with prompt: ${options.prompt.substring(0, 50)}...`);
